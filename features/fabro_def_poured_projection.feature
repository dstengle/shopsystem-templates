# ============================================================================
# RETIREMENT PROVENANCE (lead-ifye3.6, ADR-064 D1/D2) — ONE scenario in this
# feature is retired with NO successor; @scenario_hash:2786d8415362757b (the
# ADR-062 bounded-retry scenario, below) is UNRELATED and STAYS LIVE.
# The retired scenario's Given requires the poured model_stylesheet to carry the
# "MODEL_CODING"/"MODEL_REVIEW"/"MODEL_DEFAULT" fabro input placeholders and its
# When binds them via "-I MODEL_CODING" etc.; once the lead-ifye3.6 pour fix
# drops the model_stylesheet attribute (fabro >= v0.267 hard-parse-errors `{{ }}`
# there) it is unrunnable, not merely stale. Body DELETED from any live block so
# the retired hash is UNREACHABLE by block-only recompute; original body kept
# here for audit only (these `#`-comment lines are outside every canonical
# scenario region). COVERAGE NOTE: this scenario also carried the ADR-051 graph
# invariants and the ADR-049 __PLACEHOLDER__-only vault assertion; retiring it
# with no successor is a coverage gap the lead tracks separately as lead-008o8.
#
# @scenario_hash:0bc0fb71534cc0d6 RETIRED (lead-ifye3.6), superseded-by: NOTHING
# reason: model_stylesheet placeholders removed by the lead-ifye3.6 pour fix.
#   Original body (audit):
