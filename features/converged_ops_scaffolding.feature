Feature: converged ops scaffolding (lead shop, PDR-020) — bin/shop-shell is a thin bc-container-delegating wrapper, the ops set is exactly five shop-owned files with no dedicated shell Dockerfile, and a non-default render carries zero cross-product literals
  ...
  Scenario Outline: bootstrap of a "lead" shop named "<slug>" writes "bin/shop-shell" as a thin wrapper that DELEGATES the brokered Claude launch to "bc-container" running in an ephemeral bc-base — it brings up the compose supporting services, assembles the operator agent-vault "--env-file", then runs "bc-container launch" mounting the lead repo as the workspace with the lead-only docker socket and a LEAD startup-prompt override, and "bc-container attach" — while constructing NO proxy URL, fetching NO CA, building NO shell image, and mounting NO host credentials
    ...
    And the body of "bin/shop-shell" launches an ephemeral bc-base by the literal substring "docker run --rm" carrying the interactive flag literal substring "-it" and the bc-base image reference by the literal substring "shopsystem-bc-base"
    And the body of "bin/shop-shell" mounts the host docker socket into that ephemeral bc-base by the literal substring "/var/run/docker.sock:/var/run/docker.sock"
    And the body of "bin/shop-shell" mounts the lead repository into that ephemeral bc-base so the inner "bc-container" can see it
    And the body of "bin/shop-shell" inside that ephemeral bc-base invokes "bc-container launch" carrying the literal substring "--workspace-mount" and the literal substring "--mount-docker-socket", so the launched lead session mounts the live lead working tree and is granted the lead-only docker-daemon access
  ...
  @scenario_hash:5e42381f435397f2 @bc:shopsystem-templates
  ...
  Scenario: the ops scaffolding rendered by bootstrap for a non-default-slug "lead" shop carries zero cross-product SLUG-derived literals — for shop name "dummyco" the rendered "compose.yaml" contains no case-insensitive occurrence of the literal "shopsystem" and no case-insensitive occurrence of the literal "fleet", and the rendered "bin/shop-shell" contains "shopsystem" ONLY as part of the product-neutral framework image reference "shopsystem-bc-base" (per scenario 172, exempt under the compose.yaml product-neutral-image precedent) and no case-insensitive occurrence of the literal "fleet", confirming the generification leaves no default-product slug name baked into a non-default render while the shared framework image reference is preserved
    ...
    And the byte contents of "bin/shop-shell" in the target directory, after every case-insensitive occurrence of the product-neutral framework image reference literal "shopsystem-bc-base" is removed, contain no remaining case-insensitive occurrence of the literal substring "shopsystem" and no case-insensitive occurrence of the literal substring "fleet"
    And the byte contents of "bin/shop-shell" in the target directory contain the literal substring "shopsystem-bc-base", confirming the product-neutral framework image reference is preserved and not slug-rewritten to "dummyco-bc-base"
