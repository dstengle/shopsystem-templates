...
def _cmd_bootstrap(args: argparse.Namespace) -> int:
    ...
    #
    # BC-ONLY (request_bugfix lead-npm2w): the .fabro/ projection is the
    # fabro-engage realization of the BC work-loop (ADR-057 is framed
    # throughout around the BC work-loop and never contemplates a lead shop
    # receiving it). A lead is the interactive router, not a
    # workflow.fabro-running BC-work-loop shop, so the .fabro/ pour is gated on
    # shop_type == "bc" and SKIPPED entirely for shop_type == "lead" — mirroring
    # the existing shop_type branching (_skill_iterator_for, the lead ops
    # scaffolding below). The .claude/ projection above still pours for BOTH
    # shop types; only .fabro/ becomes bc-only.
    if shop_type == "bc":
        _pour_fabro(target)
...
def _cmd_update(args: argparse.Namespace) -> int:
    ...
    #
    # BC-ONLY (request_bugfix lead-npm2w): as on the bootstrap path, the .fabro/
    # projection is a BC-work-loop artifact and is gated on shop_type == "bc",
    # SKIPPED entirely for a lead shop. The .claude/ projection above still
    # (re)pours for BOTH shop types; only .fabro/ becomes bc-only.
    if shop_type == "bc":
        _pour_fabro(target)
